    ...
    Q: CustomMsg,
{
    ...
    fn approval(
        ...
        if token.owner == spender {
            let approval = cw721_soulbound::Approval {
        ...
        let filtered: Vec<_> = token
            ...
            .map(|a| cw721_soulbound::Approval {
...
fn parse_approval(item: StdResult<(Addr, Expiration)>) -> StdResult<cw721_soulbound::Approval> {
    item.map(|(spender, expires)| cw721_soulbound::Approval {
...
fn humanize_approval(approval: &Approval) -> cw721_soulbound::Approval {
    cw721_soulbound::Approval {
